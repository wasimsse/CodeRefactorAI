                    ...
                    supported_langs_html=''.join(
                        [
                            f'<span style="padding: 0.2rem 0.5rem; background: #f8f9fa; border-radius: 4px; font-size: 0.8em;">{
                                lang["icon"]} {
                                lang["name"]}</span>' for lang in config['supported_languages'].values()])),
                unsafe_allow_html=True)
    ...
    with st.container():
        ...
        with col1:
            selected_file = st.selectbox(
                ...
                files,
                index=files.index(
                    st.session_state.current_file) if st.session_state.current_file in files else 0,
                key="file_selector")
        with col2:
        ...
        if selected_file != st.session_state.current_file:
            ...
            if st.session_state.current_metrics:
                st.session_state.current_code = st.session_state.current_metrics.get(
                    'content', '')
            ...
            with tab1:
                ...
                with col1:
                    ...
                    with st.expander("View Code", expanded=True):
                        if st.session_state.current_metrics:
                            st.code(
                                st.session_state.current_metrics.get(
                                    'content', ''), language='python')
                        else:
                            st.info(
                                "No code content available for the selected file.")

                with col2:
                    st.markdown("#### Code Analysis")
                    if st.session_state.current_metrics:
                        metrics = st.session_state.current_metrics.get(
                            'metrics', {})
                        ...
                        file_ext = os.path.splitext(
                            st.session_state.current_file)[1].lower()
                        language = "Unknown"
                ...
                if st.session_state.current_code:
                    ...
                    with col3:
                        if st.button("↺ Reset", use_container_width=True):
                            st.session_state.current_code = st.session_state.uploaded_files[st.session_state.current_file].get(
                                'content', '')
                    ...
                    for i, suggestion in enumerate(
                        ...
                        with st.expander(f"Suggestion {i}: {suggestion['title']}", expanded=True):
                            ...
                            if st.button(
                                ...
                                with metric_col1:
                                    st.metric(
                                        "New Maintainability", f"{
                                            new_metrics.get(
                                                'maintainability', 0):.1f}", delta=f"{
                                            new_metrics.get(
                                                'maintainability_delta', 0):.1f}")
                                with metric_col2:
                                    st.metric(
                                        "New Complexity", f"{
                                            new_metrics.get(
                                                'complexity', 0):.1f}", delta=f"{
                                            new_metrics.get(
                                                'complexity_delta', 0):.1f}")
                else:
                    st.info(
        ...
        if st.session_state.current_file:
            ...
            with metrics_tab:
                if st.session_state.current_metrics:
                    ...
                    raw_metrics = st.session_state.current_metrics.get(
                        'raw_metrics', {})
                    ...
                    with col3:
                        ...
                        st.markdown(
                            metric_card(
                                "Packages", len(
                                    st.session_state.current_metrics.get(
                                        'imported_packages', [])), "Imported packages"), unsafe_allow_html=True)
                        comment_ratio = raw_metrics.get(
                            'comments', 0) / raw_metrics.get('loc', 1) * 100
                        st.markdown(metric_card(
                            "Comment Ratio",
                            f"{comment_ratio:.1f}%",
                    ...
                    col1, col2 = st.columns(2)
                    with col1:
                        maintainability = st.session_state.current_metrics.get(
                    ...
                    with col2:
                        complexity = st.session_state.current_metrics.get(
